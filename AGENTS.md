...
- `make all` - Alias for `cloudflare-build`. **This is the Cloudflare Pages production build command** (configured in the Cloudflare dashboard, not in this repo)
- `make cloudflare-build` - Full build: styles, feeds, and Astro build. `make all` just calls this
- `make feed` - Generate feeds (requires Python 3 with feedgen and bs4)
...
- **Python** - Atom + RSS 2.0 feed generation (`bin/feed.py`)
...
public/
  ...
  blog/           # blog static files (feed.xml, feed-rss.xml, sitemap)
  readables/      # readable content (Orson docs)
bin/
  feed.py         # Python feed generator (Atom + RSS 2.0)
```

`public/blog/feed.xml` (Atom) and `public/blog/feed-rss.xml` (RSS 2.0) are gitignored, generated
by `make feed` / `make cloudflare-build`. Not committed — regenerated on every Cloudflare build.
...
- Atom and RSS 2.0 feeds are generated by a Python script that parses Astro source files. Blog index (`/blog/`) links both via `<link rel="alternate">` in its head (Atom listed first)

### Code Style
- Never use IIFEs (immediately invoked function expressions). Write initialization code inline in the enclosing scope.
