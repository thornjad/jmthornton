          ...
          <article class="card fluid ${dropped ? 'dropped-out' : ''}">
            ...
            <h3 class="candidate">
              ...
              <small class="candidate-title">
                <em>${title}</em>&nbsp;<span class="state">${state ? '| ' + state : ''}</span>
              </small>
              <p class="info"><strong>${prospective ? 'Prospective candidate. ' : ''}</strong>${info}</p>
...
const sortCandidates = (a, b) => {
  ...
  return dropped || votes || serious || prospective || polls || a.sortName.localeCompare(b.sortName);
};
