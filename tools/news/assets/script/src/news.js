...
const addUpdatedMessage = () => {
  const msg: HTMLHeadingElement = document.createElement('h6');
  ...
  window.newsUpdateTimer = setInterval(() => {
    ...
    let timeSince: number = (timeNow - window.newsUpdateTime) / 60000;
    if (timeSince < 3600000) { // 1 hour
      ...
      window.newsUpdateTimer = setInterval(() => {
        let timeNow: number = Date.now();
        let timeSince: number = (timeNow - window.newsUpdateTime) / 3600000;
